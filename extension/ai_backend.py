...
def _is_text_type(question_type: str) -> bool:
    t = (question_type or "").lower()
    return t in ("text", "fill", "fill-in-the-blank", "short-answer", "free-text", "text-input")


def _is_code_type(question_type: str) -> bool:
    t = (question_type or "").lower()
    return t in (
        "code",
        "coding",
...
def _effective_question_type(question: str, question_type: str) -> str:
    ...
    if _is_text_type(question_type) or _is_code_type(question_type):
...
def _llm_system_prompt(question_type: str) -> str:
    if _is_code_type(question_type):
        return (
            "You are an expert programmer helping with online course coding sandboxes and labs. "
            'Respond with JSON only: {"answer": "<complete source code>", '
            '"confidence": <0.0-1.0>, "reasoning": "<brief>"}. '
            "The answer field must be the COMPLETE code that should appear in the editor: "
            "keep provided function signatures, class stubs, imports, and docstrings, then add "
            "the correct implementation body. If placeholders (?, <answer>, ___) exist, replace them. "
            "Follow the problem statement; if the docstring conflicts with the instructions, "
            "prefer the numbered instructions in the question. "
            "Do NOT include markdown fences or any explanation — only the raw code string."
        )
    if _is_text_type(question_type):
        return (
            "You answer fill-in-the-blank or short-answer academic questions. "
            'Respond with JSON only: {"answer": "<concise answer>", '
            '"confidence": <0.0-1.0>, "reasoning": "<brief>"}. '
            "The answer must be a single word or very short phrase (1-5 words) that fits the blank. "
            "Use all lowercase unless the question explicitly requires proper nouns or capitalization. "
            "Do not include any explanation inside the answer field itself."
        )
    if _is_multi_select(question_type):
        return (
...
class CourseraAI:
    ...
    async def _finalize_llm_answer_async(
        ...
        # For fill-in-the-blank and code questions, return the raw answer — no option matching.
        if _is_text_type(question_type) or _is_code_type(question_type):
            answer = raw_answers[0].strip() if raw_answers else ""
            return {
                "answer": answer,
    ...
    def _create_prompt(self, question: str, options: List[str],
        ...
        if _is_code_type(question_type):
            prompt = f"Coding sandbox exercise:\n{question}\n"
            if context:
                prompt += (
                    "\nStarter code currently in the editor "
                    "(complete / implement this — do not invent a different signature):\n"
                    f"```\n{context}\n```\n"
                )
            else:
                prompt += "\nWrite the complete correct code that answers this question.\n"
            prompt += (
                "\nReturn the COMPLETE editor contents: keep stubs/docstrings/imports and "
                "add the working implementation. Preserve structure and formatting."
            )
            return prompt
        ...
        if _is_text_type(question_type):
            ...
            prompt += (
                ...
                "Provide only the word or short phrase that belongs in the blank. "
                "Do NOT include the full sentence — just the answer that fills the blank."
            )
            return prompt
...
async def answer_question(request: QuestionRequest):
    ...
    try:
        ...
        # Fill-in-the-blank and code questions have no options — that is valid.
        is_freeform = _is_text_type(request.type) or _is_code_type(request.type)
        if not is_freeform and not any(o.strip() for o in request.options):
            raise HTTPException(status_code=400, detail="Options cannot be empty for choice questions")
